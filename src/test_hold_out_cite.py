...
from DeepRUOT.utils import group_extract, sample, to_np, generate_steps, cal_mass_loss, parser, _valid_criterions, get_base_model
...
df=pd.read_csv(DATA_DIR + '/cite.csv')
# df = df.sample(n=len(df)//3, random_state=42)
df=df.iloc[:,:dim+1]
...
f_net = FNet(in_out_dim=dim, hidden_dim=hidden_dim, n_hiddens=n_hiddens, activation='leakyrelu')
torch.save(f_net.state_dict(), os.path.join(exp_dir+'/init_model'))
...
W1_list = []
TMV_list = []
time_points = np.unique(df['samples'].values)
all_hold_out = time_points[1:-1:1]
for i in range(len(all_hold_out)):
    cur_hold_out = all_hold_out[i]

    f_net = FNet(in_out_dim=dim, hidden_dim=hidden_dim, n_hiddens=n_hiddens, activation='leakyrelu')
    f_net.to(device)
    f_net.load_state_dict(torch.load(os.path.join(exp_dir+'/init_model')))
    optimizer = torch.optim.Adam(f_net.parameters(),lr=1e-4)

    f_net,v_losses,g_losses,losses = pretrain(f_net, df,optimizer, hold_out=cur_hold_out, delta=delta, batch_size=batch_size, n_epoch=n_epoch,logger=logger,relative_mass=relative_mass)
    ...
    results = generate_trajectories_sde(df, f_net, device, exp_dir, groups, 0.0, use_mass=True, num_points=None, num_runs=1)
    df_results = pd.DataFrame(results)
    df_results = df_results[df_results["Time Point"] == cur_hold_out]

    df_results['Base Model'] = df_results['Model'].apply(get_base_model)
    df_aggregated = df_results.groupby(['Time Point', 'Base Model']).agg(
        W1_Mean=('W1 Distance', 'mean'),
        W1_STD=('W1 Distance', 'std'),
        TMV_Mean=('TMV', 'mean'),
        TMV_STD=('TMV', 'std')
    ).reset_index()
    # Extract base model name by removing the run number suffix
    df_results['Base Model'] = df_results['Model'].str.extract(r'(.+?)_run\d+$')[0]
    df_aggregated = df_results.groupby(['Time Point', 'Base Model']).agg(
        W1_Mean=('W1 Distance', 'mean'),
        W1_STD=('W1 Distance', 'std'),
        TMV_Mean=('TMV', 'mean'),
    ...
    W1_list.append(df_aggregated.loc[0, "W1_Mean"])
    TMV_list.append(df_aggregated.loc[0, "TMV_Mean"])

    df_aggregated.to_csv(os.path.join(exp_dir, 'evaluation_result.csv'), index=False)
    print(df_aggregated)

print('Hold out')
print('W1', np.mean(W1_list))
print('TMV', np.mean(TMV_list))
