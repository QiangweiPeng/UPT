...
f_net,v_losses,g_losses,losses = pretrain(f_net,df,optimizer, delta=delta, batch_size=batch_size, n_epoch=n_epoch,hold_out=None,logger=logger,relative_mass=relative_mass)
...
#         tensors = [torch.tensor(subset[col].values, dtype=torch.float32) for col in column_names]

#         data = torch.stack(tensors, dim=1)
#         with torch.no_grad():
#             t = torch.tensor([time], dtype=torch.float32)
#             _, g, _, _ = f_net(t.to(device), data.to(device))
