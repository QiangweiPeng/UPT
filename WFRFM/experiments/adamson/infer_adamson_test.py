...
from src.preprocessing import load_adamson_training_data  # noqa: E402
...
DEFAULT_RUN_DIR = (
    ...
    / "checkpoints"
    / "adamson"
    / "adamson_adamson_simulation_1_0.75_X_pca_scaled_delta10_regm1_seed42_iter3000"
)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Run Adamson test-set inference from a trained run directory. "
    ...
    parser.add_argument(
        ...
        help="Run directory produced by experiments/adamson/train_adamson_simulation.py.",
    )
    parser.add_argument(
        "--output-dir",
        type=Path,
    ...
    parser.add_argument("--n-steps", type=int, default=50, help="Integration steps used during WFR inference.")
    parser.add_argument(
        "--solver",
        type=str,
        default="euler",
    ...
    parser.add_argument("--seed", type=int, default=42, help="Random seed used for per-condition control-cell sampling.")
    parser.add_argument("--device", type=str, default=None, help="Torch device. Defaults to cuda when available, otherwise cpu.")
    parser.add_argument("--max-conditions", type=int, default=None, help="Optional cap on the number of test conditions.")
    parser.add_argument("--target-conditions", type=str, nargs="+", default=None, help="Optional explicit subset of test conditions to infer.")
    parser.add_argument(
        "--show-progress",
...
def _project_and_scale(adata_query, pca_reference, scaled_rep):
    project_pca(
...
def _reconstruct_pca_block(z_pred_scaled, pcs, mean, variance_sqrt):
    z_pred_scaled = np.asarray(z_pred_scaled, dtype=np.float32)
    z_pred = z_pred_scaled * variance_sqrt[None, :]
...
def main():
    ...
    adata_control, _, _, adata_test, _, _ = load_adamson_training_data(
        data_path=config["data_path"],
        split_path=config["split_path"],
        go_path=config.get("go_path"),
        condition_rep_path=config.get("condition_rep_path"),
    ...
    if adata_test.n_obs == 0:
        raise RuntimeError("No Adamson test cells remain after filtering to conditions present in condition_embeddings.pkl.")
    ...
    for condition in target_conditions:
        ...
        latent_pred_adata.obs["target_condition"] = condition
        ...
        latent_pred_adata.uns["prediction_metadata"]["solver"] = str(args.solver)
        latent_pred_adata.uns["prediction_metadata"]["n_steps"] = int(args.n_steps)
    ...
    if not latent_predictions:
        raise RuntimeError("No Adamson test predictions were produced.")
