...
def parse_args():
    parser = argparse.ArgumentParser(
        description=(
            "Convert Adamson perturbation gene embedding CSV into repository pickle formats."
        ),
    ...
    parser.add_argument(
        "--input-csv",
        type=Path,
        default=PROJECT_ROOT / "data" / "adamson" / "perturbation_genes_emb.csv",
        help="Input gene embedding CSV with one gene column and embedding columns.",
    )
    parser.add_argument(
        "--split-path",
        type=Path,
        default=PROJECT_ROOT / "data" / "adamson" / "splits" / "adamson_simulation_1_0.75.pkl",
        help="Adamson split file used to enumerate all condition names.",
    )
    parser.add_argument(
        "--gene-output-pkl",
        type=Path,
        default=PROJECT_ROOT / "data" / "processed" / "adamson_gene_embeddings.pkl",
        help="Output pickle mapping gene symbol to embedding vector.",
    )
    parser.add_argument(
        "--condition-output-pkl",
        type=Path,
        default=PROJECT_ROOT / "data" / "processed" / "adamson_embeddings.pkl",
        help="Output pickle mapping Adamson condition string to embedding vector.",
    )
    parser.add_argument(
        "--control-condition",
        type=str,
...
def main():
    ...
    gene_rep_dict = load_adamson_gene_rep_dict(args.input_csv)
    split_conditions = load_adamson_split_conditions(args.split_path)
    condition_names = [args.control_condition]
    for split_name in ("train", "val", "test"):
        condition_names.extend(split_conditions[split_name])

    condition_rep_dict, metadata = build_adamson_condition_rep_dict(
