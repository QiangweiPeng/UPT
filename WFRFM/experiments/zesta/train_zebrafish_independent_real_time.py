...
from src.training import (  # noqa: E402
    FNet,
    pre_compute_wfr_ot_multi_marginal_independent_real_time,
    seed_everything,
)
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Train zebrafish WFRFM with independent real-time trajectories.",
    ...
    parser.add_argument(
        ...
        type=str,
        default="gene_target",
        help="obs column used as the trajectory identifier.",
    )
    parser.add_argument(
        "--condition-rep-key",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default="timepoint",
        help="obs column that defines the ordered temporal marginals.",
    )
    parser.add_argument(
        "--control-key",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Ordered observed timepoints used when chaining trajectories.",
    )
    parser.add_argument("--seed", type=int, default=42, help="Global random seed for OT sampling and training.")
    parser.add_argument("--delta", type=float, default=FORMAL_ZEBRAFISH_PRESET["delta"], help="WFR geometry scale.")
    parser.add_argument("--reg-m", type=float, default=FORMAL_ZEBRAFISH_PRESET["reg_m"], help="Unbalanced OT mass regularization strength.")
    parser.add_argument("--group-number", type=int, default=FORMAL_ZEBRAFISH_PRESET["group_number"], help="Number of source/target groups used by mini-batch UOT.")
    parser.add_argument("--batch-save-size", type=int, default=FORMAL_ZEBRAFISH_PRESET["batch_save_size"], help="How many trajectories are precomputed before writing an intermediate OT batch file.")
    parser.add_argument("--n-iterations", type=int, default=FORMAL_ZEBRAFISH_PRESET["n_iterations"], help="Number of training iterations for flow/growth matching.")
    parser.add_argument("--batch-size-per-condition", type=int, default=FORMAL_ZEBRAFISH_PRESET["batch_size_per_condition"], help="Number of OT-matched samples drawn from each trajectory in one training step.")
    parser.add_argument("--batch-size-condition", type=int, default=FORMAL_ZEBRAFISH_PRESET["batch_size_condition"], help="Number of trajectories sampled per training step.")
    ...
    parser.add_argument("--save-interval", type=int, default=FORMAL_ZEBRAFISH_PRESET["save_interval"], help="Checkpoint save interval in training iterations.")
    parser.add_argument("--eval-interval", type=int, default=FORMAL_ZEBRAFISH_PRESET["eval_interval"], help="Validation interval in training iterations.")
    parser.add_argument("--max-conditions", type=int, default=None, help="Optional cap on the number of perturbation conditions.")
    parser.add_argument("--control-max-cells", type=int, default=None, help="Optional cap on how many control cells are loaded into memory.")
    parser.add_argument("--treated-max-cells-per-group", type=int, default=None, help="Optional cap on cells loaded for each (condition, timepoint) group.")
    parser.add_argument("--hidden-dim-v", type=int, default=FORMAL_ZEBRAFISH_PRESET["hidden_dim_v"], help="Hidden width of the velocity network.")
    parser.add_argument("--hidden-dim-g", type=int, default=FORMAL_ZEBRAFISH_PRESET["hidden_dim_g"], help="Hidden width of the growth network.")
    parser.add_argument("--n-hiddens-v", type=int, default=FORMAL_ZEBRAFISH_PRESET["n_hiddens_v"], help="Number of FiLM residual blocks in the velocity network.")
    parser.add_argument("--n-hiddens-g", type=int, default=FORMAL_ZEBRAFISH_PRESET["n_hiddens_g"], help="Number of FiLM residual blocks in the growth network.")
    parser.add_argument("--con-embedding-dim", type=int, default=FORMAL_ZEBRAFISH_PRESET["con_embedding_dim"], help="Condition encoder output dimension fed into FiLM modulation.")
    parser.add_argument("--hidden-dim-con", type=int, default=FORMAL_ZEBRAFISH_PRESET["hidden_dim_con"], help="Hidden width of the condition encoder MLP.")
    parser.add_argument("--time-dim", type=int, default=FORMAL_ZEBRAFISH_PRESET["time_dim"], help="Raw sinusoidal time encoding dimension before the learned time MLP.")
    parser.add_argument("--time-embedding-dim", type=int, default=FORMAL_ZEBRAFISH_PRESET["time_embedding_dim"], help="Final learned time embedding dimension fed into FiLM modulation.")
    parser.add_argument("--hidden-dim-time", type=int, default=FORMAL_ZEBRAFISH_PRESET["hidden_dim_time"], help="Hidden width of the time encoder MLP.")
    parser.add_argument("--bottle-dim", type=int, default=FORMAL_ZEBRAFISH_PRESET["bottle_dim"], help="Bottleneck dimension used inside each FiLM residual block.")
    parser.add_argument("--activation", type=str, default=FORMAL_ZEBRAFISH_PRESET["activation"], help="Activation function used inside the model.")
    parser.add_argument(
    ...
    parser.add_argument(
        "--recompute-ot",
        action="store_true",
        help="Force recomputation of the OT file even if it already exists.",
    )
    parser.add_argument(
        "--save-only-last",
        action="store_true",
        default=FORMAL_ZEBRAFISH_PRESET["save_only_last"],
        help="Keep only the latest checkpoint file during training.",
    )
    parser.add_argument(
        "--run-name",
        type=str,
        default=None,
        help="Optional manual run name. Defaults to an auto-generated descriptive name.",
    )
...
def build_run_name(args):
    ...
    return (
        f"zesta_independent_real_time_all_control_{rep_name}"
...
def augment_with_control_condition_embedding(data_path, adata_control, condition_rep_dict, condition_key):
    ...
    merged = dict(condition_rep_dict)
    merged.update(control_rep_dict)
    return merged
...
def main():
    ...
    condition_rep_dict = augment_with_control_condition_embedding(
        data_path=args.data_path,
        adata_control=adata_control,
    ...
    control_condition = str(adata_control.obs[args.condition_key].astype(str).unique()[0])
    control_timepoints = sorted(adata_control.obs[args.marginal_key].astype(float).unique().tolist())
    config_payload = {
    ...
    config_payload["trajectory_mode"] = "independent_real_time"
    config_payload["use_all_control"] = True
    config_payload["control_condition"] = control_condition
    ...
    summary_payload = {
        ...
        "control_timepoints": control_timepoints,
    ...
    if ot_save_path.exists() and not args.recompute_ot:
        ...
        ot_results_train = pre_compute_wfr_ot_multi_marginal_independent_real_time(
            adata_control=adata_control,
            adata_treated=adata_treated,
