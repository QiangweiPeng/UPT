...
def cal_mass_loss(data_t1, x_t_last, lnw_t_last, relative_mass, batch_size):
    ...
    distances = torch.cdist(data_t1, x_t_last)  # torch.Size([3781, 1429]) Calculate pairwise distances
    #print(distances.shape)
    _, indices = torch.min(distances, dim=1)  # Find the index of the closest point
    #print(indices)
    
    # Step 2: Compute weights from lnw_t_last
    weights = torch.exp(lnw_t_last).squeeze(1)#torch.Size([1429])
...
def compute_uot_plans(X,t_train,reg=0.05,reg_m=[50, np.inf],draw=False):# X_selected
    uot_plans = []
    ...
    for i in tqdm(range(len(t_train)-1), desc="Computing UOT plans..."):
        ...
        G = ot.unbalanced.sinkhorn_unbalanced(a, b, cost_matrix, reg_m=reg_m, reg=reg)
        uot_plans.append(G)
...
def compute_cond_g(uot_plan: np.ndarray, ratio: float, idx: np.ndarray,delta_t) -> torch.Tensor:
    selected_uot_plan = uot_plan[idx, :]  # 选取 batch 对应的行
...
###############################stage2##############################################
def generate_traj_and_weights(v, g, X0, t_start, t_end, steps=100):
    print(X0.shape)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    t_span = torch.linspace(t_start, t_end, steps)
    # print(t_span.shape)
    node = NeuralODE(torch_wrapper(v), solver="dopri5", sensitivity="adjoint", atol=1e-5, rtol=1e-5)
    w0 = torch.ones((1,X0.shape[0]), dtype=torch.float32).to(device)
    
    with torch.no_grad():
        traj = node.trajectory(
            torch.from_numpy(X0).float().to(device),
            t_span=t_span,
        ).cpu()
        points = traj[-1]
        t_span_expanded = t_span[:, None, None].repeat(1, traj.shape[1], 1)  # [num_timestep, n, 1]
        g_input = torch.cat([traj.to(device), t_span_expanded.to(device)], dim=-1)  # [num_timestep, n, d+1]
        g_values = g(g_input).squeeze(-1)  # [num_timestep, n, 1]
        log_w1 = torch.trapz(g_values, t_span, dim=0) 
        w1 = w0 * torch.exp(log_w1)
        w1_flat = w1.detach().cpu().numpy().T
        weights = w1_flat/w1_flat.sum()
    
    return traj, points, torch.tensor(weights).type_as(traj).to(device)

def get_lists(v,g,X,t_train,reg,reg_m,steps=100):
    traj_list = []
    points_list = []
    weights_list = []
    train_weights_list = []
    p_list = []
    for t in tqdm(range(len(t_train)-1), desc='Simulating trajectories, points and weights...'):
        if t == 0:
            traj, points, weights = generate_traj_and_weights(v, g, X[t], t_train[t], t_train[t+1], steps=steps)
        else:
            # local
            # traj, points, weights = generate_traj_and_weights(v, g, X[t], t_train[t], t_train[t+1], steps=steps)
            # global
            traj, points, weights = generate_traj_and_weights(v, g, points_list[t-1].cpu().numpy(), t_train[t], t_train[t+1], steps=steps)
        traj_list.append(traj)
        points_list.append(points)
        weights_list.append(weights)
        train_weights, p = get_weights_rec(points_list[t], X[t+1], reg, reg_m, weight=weights_list[t])
        train_weights_list.append(train_weights)
        p_list.append(p)
    return traj_list, points_list, train_weights_list, p_list


def get_weights_rec(points, X, reg=0.005, reg_m=20, weight=None):
    # 返回(n,)的权重,cost越大训练权重越高(注重没有拟合好的)
    source = np.array(points.detach().numpy().tolist(), dtype=object)
    target = X
    cost_matrix = ot.dist(source, target)
    cost_matrix = np.array(cost_matrix, dtype=np.float64)
    
    a = np.ones(source.shape[0]) * weight.detach().cpu().numpy().reshape(source.shape[0],)  /weight.detach().cpu().numpy().sum()
    b = np.ones(target.shape[0]) / target.shape[0]
    p = ot.unbalanced.sinkhorn_unbalanced(a,b,cost_matrix,reg=reg,method='sinkhorn',reg_m=reg_m)
    
    p_norm = p/p.sum(1,keepdims=True)
    return (cost_matrix*p_norm).sum(1)/(cost_matrix*p_norm).sum(), p_norm


def get_batch_rec(X, t_train, traj_list, points_list, train_weights_list, p_list, batch_size=128, steps=100):
    ts = []
    xts = []
    uts = []
    ws = []
    ...
    for t in range(len(t_train)-1):
        delta_t = t_train[t+1]-t_train[t]
        t_span = torch.linspace(0, delta_t, steps).to(device)
        train_weights = train_weights_list[t]
        p = p_list[t]
        x0, x1, idx = sample_from_ot_plan(p, points_list[t], torch.tensor(X[t+1]).to(device),batch_size=batch_size)
        
        # x0 = torch.from_numpy(x0).float().to(device)
        # x1 = torch.from_numpy(x1).float().to(device)
        
        t_idx = torch.randint(0, len(t_span)-1, (x0.shape[0],),device=device) #避免采到1
        t_samp = t_span[t_idx].reshape(-1,1).type_as(x0)
        xt_samp = traj_list[t][t_idx,idx,:]+0.1*torch.randn_like(x0)
        x_start = traj_list[t][0,idx,:]
        ut_samp = (x1 - x_start)/delta_t
        w_samp = torch.tensor(train_weights[idx]).type_as(x0).to(device)
        w_samp = w_samp/w_samp.sum()*batch_size
    ...
    return torch.cat(ts, dim=0), torch.cat(xts, dim=0), torch.cat(uts, dim=0), torch.cat(ws, dim=0)
