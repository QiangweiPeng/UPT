...
from .utils import sample, generate_steps, compute_uot_plans, get_batch
...
def pretrain(model, df, optimizer, n_epoch=1000, test_interval=100,
    ...
    uot_plans = compute_uot_plans(X_selected, t_train, reg=reg, reg_m=reg_m, draw=False)
...
# Specify the path to save the best model
    ...
    for batch in tqdm(range(n_batches)):
        ...
        if local_loss and not global_loss:
            i_mass=1
            lnw0 = torch.log(torch.ones(sample_size[0],1) / (initial_size)).to(device)#torch.Size([1429, 1])
            ...
            if hold_one_out:
                ...
                steps = generate_steps(groups)
            for step_idx, (t0, t1) in enumerate(steps):#一段一段开train
                print(f'batch_{batch}_{step_idx}:')
                logger.info(f'batch_{batch}_{step_idx}:')
                ...
                print(f'diff_mass:',mu.detach().cpu().numpy().sum()-relative_mass_now)
                print(f'MC:{np.abs(mu.detach().cpu().numpy().sum()-relative_mass_now)/relative_mass_now}')
                ...
                logger.info(f'MC:{np.abs(mu.detach().cpu().numpy().sum()-relative_mass_now)/relative_mass_now}')
                mu = mu / mu.sum()
                ...
                print(f'mass loss:{local_mass_loss.item():.6}')
                logger.info(f'Otloss:{loss_ot.item():.6}')
                logger.info(f'mass loss:{local_mass_loss.item():.6}')
    ...
    for batch in tqdm(range(n_batches)):
        ...
        if local_loss and not global_loss:
            ...
            for step_idx, (t0, t1) in enumerate(steps):  
                ...
                print(f'Otloss:{loss_ot.item():.6}')
                print(f'mass loss:{local_mass_loss.item():.6}')
                print(f'energy loss:{m_t_last.mean().item():.6}')
                logger.info(f'Otloss:{loss_ot.item():.6}')
                logger.info(f'mass loss:{local_mass_loss.item():.6}')
                logger.info(f'energy loss:{m_t_last.mean().item():.6}')
                ...
                if use_pinn:
                    ...
                    print(f'pinloss:{mean_pppinn_loss:.6}')
                    print(f'intial loss:{loss2:.6}')
                    logger.info(f'pinloss:{mean_pppinn_loss:.6}')
                    logger.info(f'intial loss:{loss2:.6}')
                  
                    
                    loss += lambda_pinn * mean_pppinn_loss+lambda_initial*loss2
                    print(f'total_loss:{loss.item():.6}')
                    logger.info(f'total loss:{loss.item():.6}')
